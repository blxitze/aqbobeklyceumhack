...
const NAV_LINKS = [
  { href: "/parent/dashboard", label: "Главная", icon: LayoutDashboard },
];

export default function ParentLayout({ children, parentName, childName, userEmail }: ParentLayoutProps) {
      ...
      >
        <div className="flex items-center gap-3 border-b border-white/10 px-6 py-5 pt-16 md:pt-5">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500">Родитель</div>
          </div>
        </div>
        ...
        <div className="mt-auto border-t border-white/10 px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-medium text-white">{parentName}</div>
              <div className="truncate text-xs text-slate-500">{userEmail ?? `Родитель ${childName}`}</div>
            </div>
          </div>
        </div>
        <button
